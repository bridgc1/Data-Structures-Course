...
Player::Player(int x, int y, char symbol, GameMechs* gm, ItemBin* bin, const char* control)
{
    ...
    myBin = new objPosBST(); //allocates memory on heap for root of binary search tree
}
...
{    
    ...
    if(collided)
    {
        ...
        myBin->insert(target); //if collision occurs, insert the target, aka the most recent randomly generated food item from the pregenerated stack)
        //insert function automatically sorts the nodes based on the prefix value
        // functions involved: Player:printBinContents() calls objPosBST::printTree()

        if(myBin->findGreater(gmRef->getSpecialThreshold())) //compare the max threshold set to win (number after prefix) with the max value in the tree (set 0 to 99 possible number). If any value in the BST is greater than the set threshold value, return true
        //thus this condition is only true when BST node > WIN_SPECIAL_THRES however if the win_special_thres is set greater than the max possible bst node number value then this is always false so no one wins the game (infinite)
        {
            gmRef->setWinner(this);
            gmRef->setExitFlag();
            // or, set win flag with player identification
